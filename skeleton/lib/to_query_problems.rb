...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
  SELECT
    nonshinytoys.name, nonshinytoys.price
  FROM
    toys AS shinymouse
  JOIN
    toys AS nonshinytoys
  ON
    nonshinytoys.price = shinymouse.price
  WHERE shinymouse.name = 'Shiny Mouse'
  AND nonshinytoys.name != 'Shiny Mouse'
  ORDER BY nonshinytoys.name;
...
def just_like_orange
  ...
  execute(<<-SQL)
   SELECT
    orangebreed.name , orangebreed.breed
  FROM
    cats AS orangebreed
  JOIN
    cats AS notorangebreed
  ON
    notorangebreed.breed = orangebreed.breed
  WHERE orangebreed.name != 'Orange'
  AND notorangebreed.name = 'Orange'
  ORDER BY orangebreed.name;
...
def toys_that_jet_owns_sub
  ...
  JOIN toys ON toys.id = cattoys.toy_id
  WHERE
    cats.name != 'Jet' AND toys.name IN (
      SELECT DISTINCT
        toys.name
      FROM
        cats
